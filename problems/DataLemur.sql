...
(SELECT DISTINCT c1.caller_id, i1.country_id FROM phone_calls c1
  INNER JOIN phone_info i1
  ON c1.caller_id = i1.caller_id
  ORDER BY c1.caller_id),

receiver AS
(SELECT DISTINCT c2.receiver_id, i2.country_id FROM phone_calls c2
  INNER JOIN phone_info i2
  ON c2.receiver_id = i2.caller_id
  ORDER BY c2.receiver_id),

  combined AS
  (SELECT pc.caller_id, pc.receiver_id,
  ca.country_id AS caller_country, re.country_id AS receiver_country 
  FROM phone_calls pc

  INNER JOIN caller ca
  ON pc.caller_id = ca.caller_id
  INNER JOIN receiver re
  ON pc.receiver_id = re.receiver_id)
  
  SELECT
  
  ROUND(SUM(CASE WHEN caller_country != receiver_country THEN 1 ELSE 0 END)*100.0/
  (SELECT COUNT(caller_id) FROM combined),1) AS international_calls_pct
...
SELECT ROUND(
         100.0 * SUM(CASE WHEN ci.country_id <> ri.country_id THEN 1 ELSE 0 END)
         / COUNT(*),
...
-- Q45: Google's marketing team is making a Superbowl commercial and needs a simple statistic to put on their TV ad: the median number of searches a person made last year.
...
WITH combined AS(
  ...
  combined_status AS (
    SELECT c.user_id, a.status FROM combined c
    LEFT JOIN advertiser a
    ON c.user_id = a.user_id),
  
  current_status AS (
  SELECT
  cc.user_id, cc.status, d.paid
  FROM combined_status cc
  LEFT JOIN daily_pay d
  ON cc.user_id = d.user_id)
  
SELECT
  user_id,
  (CASE WHEN paid ISNULL THEN 'CHURN' ELSE
    CASE WHEN status IN ('NEW', 'EXISTING', 'RESURRECT')
    THEN 'EXISTING' ELSE
      CASE WHEN status = 'CHURN' THEN 'RESURRECT' ELSE
        CASE WHEN status ISNULL THEN 'NEW' 
        END
      END
    END
  END) AS new_status
